...
class PMRoomGenerator(RoomGenerator):
    ...
    def shell_config(self, start_pos=(0,0), size=1):
        ...
        particle.geom(name='particle_geom', type='sphere', size=size_of_particle, rgba='1.0 1.0 1.0 1', contype=1)
        ...
        actuator.motor(joint="ball_x", ctrlrange=[-1.0, 1.0], gear=0.3, ctrllimited=True)
        actuator.motor(joint="ball_y", ctrlrange=[-1.0, 1.0], gear=0.3, ctrllimited=True)
...
class Room(Serializable):
    ...
    def __init__(self, base='pm', length=1.2, width=1.2, start=None, target=None, starting_length=None, starting_width=None):
        ...
        if start is None:
            start = np.array((- self.length / 2 + 0.2,0))
        if target is None:
            target = np.array((self.length / 2 - 0.2, 0))
    ...
    def get_starting_boundary(self):
        bottom_left = (-self.starting_length / 2, - self.starting_width / 2) 
