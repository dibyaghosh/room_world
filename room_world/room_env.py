# Base imports
import numpy as np
from room_world.abstractions import IdentityEmbedding, RewardAbstraction
...
class RoomEnv(MujocoEnv, MultitaskEnv, Serializable):
    ...
    def __init__(self,
        ...
        self.obs_space = Box(-1 * np.inf * np.ones(obs_size), np.inf * np.ones(obs_size))
        self.state_obs_space = Box(-1 * np.inf * np.ones(state_obs_size), np.inf * np.ones(state_obs_size))
        ...
        self.goal_space = Box(-1 * np.inf * np.ones(goal_size), np.inf * np.ones(goal_size))
        self.state_goal_space =  Box(-1 * np.inf * np.ones(state_goal_size), np.inf * np.ones(state_goal_size))
    ...
    def compute_rewards(self, actions, obs):
        achieved_goals = self.goal_embedding.get_states(np.array(obs['achieved_goal']))
        desired_goals = self.goal_embedding.get_states(np.array(obs['desired_goal']))
        rewards = np.array([
    ...
    def _get_obs(self):
        ...
        achieved_goal = self._get_env_achieved_goal(state_obs).copy()
        intended_goal = self.goal.copy()

        return dict(
            observation=self.state_embedding.get_embedding(state_obs),
            desired_goal=self.goal_embedding.get_embedding(intended_goal),
            achieved_goal=self.goal_embedding.get_embedding(achieved_goal),
            state_observation=state_obs,
            state_desired_goal=intended_goal,
    ...
    def get_goal(self):
        return {
            'desired_goal': self.goal_embedding.get_embedding(self.goal),
    ...
    def sample_goals(self, batch_size):
        ...
        transformed_goals = [self.goal_embedding.get_embedding(goal) for goal in goals]
    ...
    def get_states(self, max_size=100):
        raise NotImplementedError()
        
    def get_goals(self, max_size=100):
        raise NotImplementedError()
    ...
    def set_param_values(self, params):
        self.start_config = params['start_config']
        self.goal_config = params['goal_config']
        self._room = params['room']
